<?php
    $judul_halaman = "Edit Kategori Produk";
    
    include '../../cek_login.php';

    // Ambil data kategori berdasarkan ID
    $id = $_GET['id'];
    $query = "SELECT * FROM kategori_produk WHERE id_kategori_produk = '$id' AND status_dihapus = 0";
...
<?php include '../../layout/sidebar.php'; ?>
    ...
    <main class="flex-1 p-2 sm:p-4 lg:p-6 animate-slide-in-down">
        <div class="bg-gray-100 rounded-lg shadow-md p-6 animate-slide-in-up">
            <h2 class="text-2xl font-semibold mb-6 text-pink-600 animate-bounce-in">Edit Kategori</h2>
            ...
            <input type="hidden" name="id" value="<?php echo $data['id_kategori_produk']; ?>">
            <div class="form-group animate-slide-in-left">
                <label class="block text-sm font-medium text-indigo-500 mb-2">Nama Kategori</label>
                <input type="text" name="nama_kategori" value="<?php echo $data['nama_kategori']; ?>" class="w-full px-3 py-2 border border-pink-200 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-400" placeholder="Masukkan nama kategori" required>
            </div>
