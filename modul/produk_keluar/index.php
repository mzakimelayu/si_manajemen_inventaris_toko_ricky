<?php
    $judul_halaman = "Data Produk Keluar";
...
<?php include '../../layout/sidebar.php'; ?>
          ...
          <main class="flex-1 p-2 sm:p-4 lg:p-6 bg-gradient-to-br from-pink-50 to-indigo-50 animate-slide-in-up">
              <div class="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 animate-scale-in">
                  <div class="p-4 sm:p-6 lg:p-8 bg-gradient-to-r from-pink-500 via-rose-500 to-purple-500 text-white rounded-t-xl animate-slide-in-down">
                      <div class="flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0">
                          ...
                          <a href="tambah.php" class="w-full sm:w-auto bg-pink-600 hover:bg-rose-500 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors duration-300 shadow-md hover:shadow-lg animate-slide-in-right">
                              ...
                              <span>Tambah Produk Keluar</span>
                          </a>
                      </div>
                  </div>
                  ...
                  <div class="p-3 sm:p-4 lg:p-6 animate-slide-in-up">
                        ...
                        <?php
                            if(isset($_SESSION['produk_keluar_sukses'])) { ?>
                            <div id="alert-message" class="mb-4 bg-gradient-to-br from-green-50 via-emerald-100 to-teal-50 p-4 rounded-xl shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 flex justify-between items-center animate-pulse" role="alert">
                                ...
                                <div>
                                    ...
                                    <p class="text-green-600 text-sm"><?php echo $_SESSION['produk_keluar_sukses']; ?></p>
                                </div>
                                </div>
                        ...
                        <?php 
                        unset($_SESSION['produk_keluar_sukses']);
                        } 
                        ?>
                      <div class="flex flex-col sm:flex-row gap-4 mb-6">
                          <div class="flex-1">
                              <div class="relative animate-slide-in-left">
                                  <input type="text" id="searchInput" class="w-full pl-10 pr-4 py-2 bg-pink-200 border border-pink-300 rounded-lg focus:ring-2 focus:ring-pink-400 focus:border-pink-400 focus:outline-none text-sm sm:text-base" placeholder="Cari pengeluaran...">
                          ...
                          <table id="penggunaTable" class="min-w-full">
                              <thead>
                                  <tr class="bg-pink-200">
                                      <th class="px-4 py-3 border-b border-r border-rose-300 text-center text-xs sm:text-sm font-semibold text-indigo-500 cursor-pointer" data-column="id_pengeluaran_produk">
                                      ...
                                      <th class="px-4 py-3 border-b border-r border-rose-300 text-center text-xs sm:text-sm font-semibold text-indigo-500 cursor-pointer" data-column="penerima">
                                          <div class="flex items-center justify-center gap-1">
                                              <span>Penerima</span>
                                              <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/>
                                      ...
                                      <th class="px-4 py-3 border-b border-r border-rose-300 text-center text-xs sm:text-sm font-semibold text-indigo-500 cursor-pointer" data-column="status_dihapus">
                                          <div class="flex items-center justify-center gap-1">
                                              <span>Status</span>
                                              <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/>
          ...
          let currentPage = 1;
          let itemsPerPage = 10;
          let sortColumn = 'id_pengeluaran_produk';
          let sortDirection = 'asc';
          
          async function fetchData() {
              try {
                  const response = await fetch('get_produk_keluar.php');
          ...
          function renderTable(data = tableData) {
              const groupedData = data.reduce((acc, curr) => {
                  if (!acc[curr.id_pengeluaran_produk]) {
                      acc[curr.id_pengeluaran_produk] = {
                          ...curr,
                          products: [{
                              kode_produk: curr.kode_produk,
                              nama_produk: curr.nama_produk,
                              harga_jual: curr.harga_jual,
                              jumlah: curr.jumlah
                          }]
                      };
                  } else {
                      acc[curr.id_pengeluaran_produk].products.push({
                          kode_produk: curr.kode_produk,
                          nama_produk: curr.nama_produk,
                          harga_jual: curr.harga_jual,
                          jumlah: curr.jumlah
              ...
              paginatedData.forEach((row, index) => {
                  ...
                  let productsHtml = row.products.map(product => `
                      ${product.kode_produk} - ${product.nama_produk} (${product.jumlah} @ Rp${product.harga_jual})
                  `).join('<br>');
                  ...
                  tr.innerHTML = `
                      ...
                      <td class="px-4 py-3 border-b border-r border-rose-300">${row.penerima}</td>
                      ...
                      <td class="px-4 py-3 border-b border-rose-300">
                          <div class="flex justify-center space-x-3">
                              <button onclick="lihatPengeluaran(${row.id_pengeluaran_produk})" class="text-fuchsia-300 hover:text-fuchsia-300 p-1.5 rounded-full hover:bg-pink-200 group" title="Lihat Detail Produk Keluar">
                                  <div class="absolute z-10 hidden group-hover:block bg-gray-900 text-white text-xs rounded-lg py-1 px-3 -mt-14 -ml-16">
                                      Lihat Detail Produk Keluar
                                  </div>
                              ...
                              <button onclick="editPengeluaran(${row.id_pengeluaran_produk})" class="text-pink-400 hover:text-pink-600 p-1.5 rounded-full hover:bg-pink-200 group" title="Edit Data Produk Keluar">
                                  <div class="absolute z-10 hidden group-hover:block bg-gray-900 text-white text-xs rounded-lg py-1 px-3 -mt-14 -ml-16">
                              ...
                              <button onclick="hapusPengeluaran(${row.id_pengeluaran_produk})" class="text-rose-500 hover:text-rose-500 p-1.5 rounded-full hover:bg-pink-200 group" title="Batalkan Data Produk Keluar">
                                  <div class="absolute z-10 hidden group-hover:block bg-gray-900 text-white text-xs rounded-lg py-1 px-3 -mt-14 -ml-16">
                                      Batalkan Data Produk Keluar
                                  </div>
                                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                  </svg>
              ...
              if (confirm('Apakah Anda yakin ingin menghapus data pengeluaran ini?')) {
                  try {
                      ...
                      if (data.success) {
                          ...
                          alert(data.message || 'Gagal menghapus data pengeluaran');
                      }
                  } catch (error) {
                      console.error('Error:', error);
                      alert('Terjadi kesalahan saat menghapus data pengeluaran');
                  }
              }
          }
