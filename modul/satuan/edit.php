<?php
    $judul_halaman = "Edit Satuan Produk";
    
    include '../../cek_login.php';

    // Ambil data satuan berdasarkan ID
    $id = $_GET['id'];
    $query = "SELECT * FROM satuan_produk WHERE id_satuan_produk = '$id' AND status_dihapus = 0";
...
<?php include '../../layout/sidebar.php'; ?>
    ...
    <main class="flex-1 p-2 sm:p-4 lg:p-6 animate-slide-in-down">
        <div class="bg-gray-100 rounded-lg shadow-md p-6 animate-slide-in-up">
            <h2 class="text-2xl font-semibold mb-6 text-pink-600 animate-bounce-in">Edit Satuan</h2>
                ...
                <div id="alert-message" class="mb-4 bg-gradient-to-br from-red-50 via-red-100 to-orange-50 p-4 rounded-xl shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 flex justify-between items-center animate-pulse animate-slide-in-right" role="alert">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-red-600 text-sm"><?php echo $_SESSION['satuan_eror']; ?></p>
                        </div>
                    </div>
            ...
            <input type="hidden" name="id" value="<?php echo $data['id_satuan_produk']; ?>">
            <div class="form-group animate-slide-in-left">
                <label class="block text-sm font-medium text-indigo-500 mb-2">Nama Satuan</label>
                <input type="text" name="nama_satuan" value="<?php echo $data['nama_satuan']; ?>" class="w-full px-3 py-2 border border-pink-200 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-400" placeholder="Masukkan nama satuan" required>
            </div>
