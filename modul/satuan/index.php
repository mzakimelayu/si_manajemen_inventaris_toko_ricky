<?php
    $judul_halaman = "Data Satuan Produk";
...
<?php include '../../layout/sidebar.php'; ?>
                ...
                <?php
                    ...
                    <div id="alert-message" class="mb-4 bg-gradient-to-br from-green-50 via-emerald-100 to-teal-50 p-4 rounded-xl shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 flex justify-between items-center animate-pulse" role="alert">
                        ...
                        <div>
                            ...
                            <p class="text-green-600 text-sm"><?php echo $_SESSION['satuan_sukses']; ?></p>
                        </div>
                        </div>
                ...
                <div class="flex flex-col sm:flex-row gap-4 mb-6">
                    <div class="flex-1">
                        <div class="relative animate-slide-in-left">
                            <input type="text" id="searchInput" class="w-full pl-10 pr-4 py-2 bg-pink-200 border border-pink-300 rounded-lg focus:ring-2 focus:ring-pink-400 focus:border-pink-400 focus:outline-none text-sm sm:text-base" placeholder="Cari satuan...">
    ...
    <script>
        ...
        let currentPage = 1;
        let itemsPerPage = 10;
        let sortColumn = 'id_satuan';
        let sortDirection = 'asc';
        
        async function fetchData() {
            try {
                const response = await fetch('get_satuan_produk.php');
                ...
                tr.innerHTML = `
                    ...
                    <td class="px-4 py-3 border-b border-rose-300">
                        <div class="flex justify-center space-x-3">
                            <button onclick="editSatuan(${row.id_satuan})" class="text-pink-400 hover:text-pink-600 p-1.5 rounded-full hover:bg-pink-200 group" title="Edit Data Satuan">
                            ...
                            <button onclick="hapusSatuan(${row.id_satuan})" class="text-rose-500 hover:text-rose-500 p-1.5 rounded-full hover:bg-pink-200 group" title="Hapus Data Satuan">
                                <div class="absolute z-10 hidden group-hover:block bg-gray-900 text-white text-xs rounded-lg py-1 px-3 -mt-14 -ml-16">
                                    Hapus Data Satuan
                                </div>
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            if (confirm('Apakah Anda yakin ingin menghapus satuan ini?')) {
                try {
                    ...
                    if (data.success) {
                        ...
                        alert(data.message || 'Gagal menghapus satuan');
                    }
                } catch (error) {
